...
def downloadTools():
    ...
    # os.system(f"cd {softwareDir}/bwa-0.7.17 ; make -s" )
    # print("~~~> Testing BWA...")
    ...
    # if output[0].strip() == "":
    #     cmd = f"cd {softwareDir}/STAR-2.7.9a/source ; make STAR CXXFLAGS_SIMD=sse -s"
    # else: 
    #     cmd = f"cd {softwareDir}/STAR-2.7.9a/source ; make STAR -s"
    # os.system(cmd)
    # print("~~~> Testing STAR...")
    ...
    util.execCmd("wget https://versaweb.dl.sourceforge.net/project/snpeff/snpEff_v4_3t_core.zip")
    util.execCmd(f"unzip -o -q snpEff_v4_3t_core.zip -d {softwareDir}")
    util.execCmd("rm snpEff_v4_3t_core.zip")
    print("~~~> Testing SnpEff...")
    output = util.execCmd(f"{softwareDir}/java-se-8u42-ri/bin/java -jar {softwareDir}/clinEff/ClinEff.jar")
    if output[1].split("\n")[0].strip() != "ClinEff version ClinEff 1.0h (build 2017-11-24 10:19), by Pablo Cingolani":
        print("~~~> Test failed.")
        print(output[1])
        util.stopProgram()
    else:
