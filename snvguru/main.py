...
def run_init():
    ...
    if os.path.exists(dest_config):
        print(f"Warning: '{dest_config}' directory already exists. Skipping config initialization to prevent overwriting.")
    else:
        print("Initializing configuration files under './config/'...")
        shutil.copytree(os.path.join(templates_dir, "config"), dest_config)
        
    input_files = ["singleInput.txt", "pairedInput.txt", "mixedInput.txt", "sras.txt", "projects.txt"]
    for f in input_files:
        if os.path.exists(f):
            print(f"Warning: '{f}' already exists in the current directory. Skipping.")
        else:
            print(f"Initializing '{f}'...")
            shutil.copy(os.path.join(templates_dir, f), f)
            
    print("\nInitialization complete! Customize 'config/main.config' and run your analysis.")
...
def main():
    ...
    from snvguru import arguments
    from snvguru import downloads
    from snvguru import quality
    from snvguru import cropping
    from snvguru import alignment
    from snvguru import calling
    from snvguru import results
    from snvguru import report
